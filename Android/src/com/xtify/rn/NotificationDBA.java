...
public class NotificationDBA {
	...
	public void removeAllNotifs() {
		SQLiteDatabase db = DBAdapter.getSQLiteDb(context);
		db.delete(DATABASE_TABLE_NOTIFS, null, null); // If whereClause is null,
														// it will delete all
														// rows.
	}
	...
	public ArrayList<RichNotification> getAllNotifs() {
		SQLiteDatabase db = DBAdapter.getSQLiteDb(context);
		Cursor cur = db.query(DATABASE_TABLE_NOTIFS, new String[] { KEY_ROWID, "actionData", "actionLabel", "actionType", "content", "date",
				"subject", "ruleLat", "ruleLon", "mid", "expirationDate" }, null, null, null, null, null);
		...
		while (cur.isAfterLast() == false) {
			...
			String timestamp = com.payless.utils.Calendar.getInstance().getTime().toString();
			try {
			...
			richNotifList.add(new RichNotification(cur.getString(1), cur.getString(2), cur.getString(3), cur.getString(4), timestamp, cur
					.getString(6), cur.getDouble(7), cur.getDouble(8), cur.getString(9), cur.getString(10)));
			cur.moveToNext();
		}
		cur.close();
	...
	public RichNotification getNotifByMid(String mid) throws SQLException {
		SQLiteDatabase db = DBAdapter.getSQLiteDb(context);
		Cursor cur = db.query(true, DATABASE_TABLE_NOTIFS, new String[] { KEY_ROWID, "actionData", "actionLabel", "actionType", "content", "date",
				"subject", "ruleLat", "ruleLon", "mid", "expirationDate" }, "mid" + "='" + mid + "'", null, null, null, null, null);
		if (cur != null) {
			cur.moveToFirst();
		}
		RichNotification rn = new RichNotification(cur.getString(1), cur.getString(2), cur.getString(3), cur.getString(4), cur.getString(5),
				cur.getString(6), cur.getDouble(7), cur.getDouble(8), cur.getString(9), cur.getString(10));
		cur.close();
		return rn;
